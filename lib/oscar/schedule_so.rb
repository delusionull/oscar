  ...
  class ScheduleSO 
    ...
    def flag_all_lines_complete
      ...
      wos_and_qty.each do |wo_and_qty|
        qty_done = qty_accepted(wo_and_qty[0], layup_lines)
    ...
    def flag_all_lines_printed
      ...
      wos_and_qty.each do |wo_and_qty|
        flag = '' unless printed?(wo_and_qty[0], layup_lines)
      end
      flag
    end
    ...
    def lines_with_done_and_total
      the_lines = []
      wos_and_qty.each do |wo_and_qty|
        line_num = find_line(wo_and_qty[0])
        qty_done = qty_accepted(wo_and_qty[0], layup_lines)
        prt = "#{printed?(wo_and_qty[0], layup_lines) ? '_p' : ''}"
        the_lines << "#{line_num}_#{wo_and_qty[0]}_#{qty_done}/#{wo_and_qty[1]}#{prt}"
      end
      the_lines.sort
    end
