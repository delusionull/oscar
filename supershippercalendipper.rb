...
FROM   (((so1_soentryheader eh
  ...
  INNER JOIN so_03sohistoryheader
    ...
    AND ( so_04sohistorydetail.salesordernumber =
...
elev_ary = []
load_elev_ary =
  Proc.new { |line| elev_ary << line[:SalesOrder] if line[:SOType] == "ele" }
...
purchase_orders = DB_MAS90[:PO1_PurchaseOrderEntryHeader]
  .select(:PurchaseOrderNumber, :Comment, :RequiredExpireDate)
po_material_items = DB_MAS90[:PO2_PurchaseOrderEntryLine]
  .select(:ItemNumber, :QtyOrdered, :UnitCost)
so_line_material = DB_MAS90[:SO_04SOHistoryDetail]
  .select(:SalesOrderNumber, :ItemNumber, :RevisedOrderQuantity)

# column_headers = "SalesOrder,Changes,ShipDate,SOType,Hld,Customer,APDMove," +
#   "Status1,Status2,Status3,MTG Notes,Job Notes,OPS Notes,CSR Notes," +
#   "Shipping Notes,Purchasing Notes,Material POs,Material,Dollars,Weight," +
#   "ShipVia,City,State,Sales"
# puts column_headers

fsc_orders = DB_MAS90[:SO_04SOHistoryDetail].select(:SalesOrderNumber)
  .where(Sequel.like(:ItemNumber, "%PREFSC%"))
