function checkForNewData() {
  if (!(fileExistsOnGDriveRoot(sscd_data_file_name))) {
    toaster("File: \"" + sscd_data_file_name + "\" not found. Upload, please.", "No Data");
    return false;
  }
}
...
function insertLookupFormulas(sheet, row) {
  ...
  sheet.getRange(row, toNum(weightperdate_column), 1, 1)
    .setFormula("=if(AND(\n" +
                "    indirect(\"$C\"&(row())) = INDIRECT(\n" +
                "      ADDRESS(ROW()-1,3,3)),\n" +
                "    indirect(\"$U\"&(row())) = INDIRECT(\n" +
                "      ADDRESS(ROW()-1,21,3))\n" +
                "    ),\n" +
                "  \"\",\n" +
                "  SUMIFS(\n" +
                "    indirect(\"$T\"&(row())&\":T\"),\n" +
                "    indirect(\"$C\"&(row())&\":C\"),\"=\" & indirect(\"$C\"&(row())),\n" +
                "    indirect(\"$U\"&(row())&\":U\"),\"=\" & indirect(\"$U\"&(row()))\n" +
                "  )\n" +
                ")\n")
}
...
function setReferenceCells(sscd_sheet) {
  sscd_sheet.getRange("Z1").setValue(isoDateString(_, 1));
}

function fancySort(sheet_to_sort) {
  var ss_range = sheet_to_sort.getRange("A2:AA");
